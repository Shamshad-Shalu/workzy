...
import { BILL_TYPE, PAYMENT_STATUS, type PaymentStatus } from '@/constants/payment';
import TableSkeleton from '@/features/admin/payments/components/TableSkeleton';
...
export default function UserPaymentsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 md:py-10">
      <motion.div
      ...
      <div className="bg-card border  rounded-xl ">
        <div className="max-h-[70vh] overflow-y-auto">
          {isLoading ? (
            ...
            >
              <div className="grid grid-cols-[1.5fr_1.2fr_1.2fr_1fr_0.8fr_auto] px-5 py-3 border-b border-border/50 bg-muted/50">
                {[
              ...
              <div>
                ...
                {isFetchingNextPage &&
                  Array.from({ length: 3 }).map((_, i) => (
                    <div
                      key={i}
                      className="grid grid-cols-[1.5fr_1.2fr_1.2fr_1fr_0.8fr_auto] px-5 py-4 border-b border-border/50 items-center gap-4"
                    >
...
function PaymentRow({ payment, index }: { payment: PaymentUser; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group grid grid grid-cols-[1.5fr_1.2fr_1.2fr_1fr_0.8fr_auto] px-5 py-3.5 border-b border-border/50 text-sm items-center hover:bg-muted/40 transition-colors duration-150"
    >
      <div className="flex flex-col gap-0.5">
        {payment.billType === BILL_TYPE.BOOKING && (
          <Link
            to={`/bookings/${payment.id}`}
