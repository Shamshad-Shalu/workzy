...
import type { PaymentWorker } from '@/types/payment';
      ...
      <div className="bg-card border  rounded-xl ">
        <div className="max-h-[70vh] overflow-y-auto">
          {isLoading ? (
            ...
            >
              <div className="grid grid-cols-[1.5fr_1.2fr_1fr_1fr_0.8fr_0.8fr_auto] px-5 py-3 border-b border-border/50 bg-muted/50">
                {[
...
function PaymentRow({ payment, index }: { payment: PaymentWorker; index: number }) {
    ...
    >
      <div className="flex flex-col gap-0.5">
        {payment.billType === BILL_TYPE.BOOKING && (
          <Link
            to={`/worker/bookings/${payment.refId}`}
      ...
      <div>
        <span className="text-md text-primary">{payment.userName}</span>
        <br />
      </div>
      <div className="flex flex-col gap-0.5">
        <span className="font-semibold text-foreground">
          ₹
          {payment?.workerAmount?.toLocaleString('en-IN') ?? payment.amount.toLocaleString('en-IN')}
        </span>
      </div>
      <div>
