...
export function NotificationsDropdown() {
  ...
  const getDynamicIcon = (iconName?: string, color?: string) => {
    ...
    const IconCmp = (LucideIcons as any)[iconName || 'Bell'] || Bell;
    return (
      <div
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        <div className="flex flex-col bg-background/95 backdrop-blur-xl rounded-2xl overflow-hidden">
          <div className="px-5 py-4 flex flex-row items-center justify-between border-b border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
            <div>
          ...
          <div className="flex items-center gap-2 px-5 py-3 border-b border-border/50 bg-muted/20">
            <Button
              ...
              className={`rounded-full h-8 px-4 text-xs font-medium transition-all ${filter === 'unread' ? 'shadow-sm' : 'bg-transparent hover:bg-accent'}`}
            >
              Unread
              {unreadCount > 0 && (
                <Badge
            ...
            <Button
              ...
              className={`rounded-full h-8 px-4 text-xs font-medium transition-all ${filter === 'all' ? 'shadow-sm' : 'bg-transparent hover:bg-accent'}`}
            >
              All
            </Button>
          </div>
          ...
          <div className="p-0 relative">
            {isLoading && notifications.length === 0 ? (
              ...
              <div
                ref={scrollRef}
                className="max-h-[450px] overflow-y-auto p-3 space-y-2 custom-scrollbar"
              >
                {notifications.map(n => (
                  <div
                    key={n._id}
                  ...
                  >
                    ...
                    {getDynamicIcon(n.notificationTypeId?.icon, n.notificationTypeId?.iconColor)}

                    <div className="flex-1 space-y-1 min-w-0 pr-6">
                      <div className="flex justify-between items-start gap-2">
                        <h4
                          className={`text-[13px] leading-tight ${!n.read ? 'font-semibold text-foreground' : 'font-medium text-muted-foreground'}`}
                        >
                          {n.heading}
                        </h4>
                      ...
                      <p
                        className={`text-[13px] leading-[1.5] ${!n.read ? 'text-muted-foreground' : 'text-muted-foreground/70'}`}
                      >
